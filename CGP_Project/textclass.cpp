						   ...
						   D3DXMATRIX baseViewMatrix)
{
	...
	result = m_Font->Initialize(device, "../Engine/data/ETC/fontdata.txt", L"../Engine/data/ETC/font.dds");
	if(!result)
	{
	...
	result = UpdateSentence(m_sentence1, "", 100, 100, 1.0f, 1.0f, 1.0f, deviceContext);
	if(!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, "", 100, 200, 1.0f, 1.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence3, "", 100, 300, 0.f, 0.f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence4, "", 100, 400, 1.0f, 0.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence5, "", 100, 500, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Initialize the first sentence.
	result = InitializeSentence(&m_sentence6, 64, device);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence5, "", 100, 600, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetPolyCnt(int polyCnt, ID3D11DeviceContext *deviceContext)
{
	char tempString[16];
	char countString[32];
	bool result;
	...
	// Convert the cpu integer to string format.
	_itoa_s(polyCnt, tempString, 10);

	// Setup the cpu string.
	strcpy_s(countString, "Polygon #: ");
	strcat_s(countString, tempString);

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence3, countString, 20, 60, 0.0f, 0.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetObjCnt(int ObjCnt, ID3D11DeviceContext *deviceContext)
{
	char tempString[16];
	char countString[16];
	bool result;
	...
	// Convert the cpu integer to string format.
	_itoa_s(ObjCnt, tempString, 10);

	// Setup the cpu string.
	strcpy_s(countString, "Object #: ");
	strcat_s(countString, tempString);

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence4, countString, 20, 80, 1.0f, 0.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
{
	char tempString[16];
	char ScreenXString[32];
	char ScreenYString[32];
	bool result;

	_itoa_s(ScreenX, tempString, 10);

	strcpy_s(ScreenXString, "ScreenWidth: ");
	strcat_s(ScreenXString, tempString);

	result = UpdateSentence(m_sentence5, ScreenXString, 20, 100, 0.f, 1.f, 0.f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	strcpy_s(ScreenYString, "ScreenHeight: ");
	strcat_s(ScreenYString, tempString);

	result = UpdateSentence(m_sentence6, ScreenYString, 20, 120, 0.f, 1.f, 0.f, deviceContext);
	if (!result)
	{
		return false;
	}
